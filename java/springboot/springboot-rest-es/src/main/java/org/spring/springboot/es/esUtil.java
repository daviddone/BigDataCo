package org.spring.springboot.es;
	...
	public static  Client getClient() {
		...
		try {
			client = TransportClient.builder().settings(settings).build()
					.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("104.140.18.185"), 9300));//客户端采用9300端口
//					.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("slave1"), 9300)) //hostname
	...
	public static Map<String, Object> search(String key,String index,String type,int start,int row){
		...
		if(!"".equals(key)){
//			builder.setQuery(QueryBuilders.matchQuery(key, queryString)) //单个关键字查询
			builder.setQuery(QueryBuilders.multiMatchQuery(key, "title","desc"));
		}
		builder.setExplain(true);//是否按查询匹配度排序
