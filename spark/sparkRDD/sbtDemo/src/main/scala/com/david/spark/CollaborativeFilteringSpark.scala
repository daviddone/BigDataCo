...
object CollaborativeFilteringSpark {
  ...
  val users = sc.parallelize(Array("aaa", "bbb", "ccc", "ddd", "eee"))//初始化用户
  val films = sc.parallelize(Array("smzdm", "ylxb", "znh", "nhsc", "fcwr")) //初始电影名

  val source = Map[String, Map[String, Int]]()// map存储 用户名,（电影名，分值）
